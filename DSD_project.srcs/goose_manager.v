...
module goose_manager (
    ...
    // Dynamically calculate how many pixels the world moved
    wire [10:0] scroll_amount = world_scrolled ? (logical_world_y - prev_world_y) : 11'd0;
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            if (spawn_pulse) begin
                if (lfsr_rand[1:0] == 2'b00) begin
                    if (!active[0]) begin
                        active[0] <= 1; scared[0] <= 0; 
                        y_pos[0] <= spawn_y + scroll_amount; 
                        x_pos[0] <= (lfsr_rand[2]) ? SCREEN_LEFT : SCREEN_RIGHT;
                    end else if (!active[1]) begin
                        active[1] <= 1; scared[1] <= 0; 
                        y_pos[1] <= spawn_y + scroll_amount;
                        x_pos[1] <= (lfsr_rand[3]) ? SCREEN_LEFT : SCREEN_RIGHT;
                    end else if (!active[2]) begin
                        active[2] <= 1; scared[2] <= 0; 
                        y_pos[2] <= spawn_y + scroll_amount;
            ...
            if (shout_detected) begin
                ...
                goose_bonus <= ((active[0] && !scared[0]) ? 6'd10 : 6'd0) + 
                               ((active[1] && !scared[1]) ? 6'd10 : 6'd0) + 
                               ((active[2] && !scared[2]) ? 6'd10 : 6'd0);
            ...
            for (i = 0; i < 3; i = i + 1) begin
                if (active[i]) begin
                    if (scared[i]) begin
                        y_pos[i] <= y_pos[i] + scroll_amount - 11'd2; 
                      ...
                      y_pos[i] <= y_pos[i] + scroll_amount; 
                        
                        if (y_pos[i] > 11'd800) active[i] <= 0;

                        if (y_pos[i] == player_y) begin 
