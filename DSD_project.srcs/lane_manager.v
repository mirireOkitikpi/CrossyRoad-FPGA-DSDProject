...
// Description: Procedurally generates and manages the 6-lane map.
// Features: True free-running PRNG, 3-bit lane types (Chasm), and 
// smart generation to prevent unfair River/Chasm combos.
//////////////////////////////////////////////////////////////////////////////////
    ...
    function [2:0] gen_lane_type;
        ...
        begin
            if (consec >= 2'd3)
                ...
                3'b110, 3'b111: begin
                    //ensures a chasm is not spawned after a chasm
                    if (prev_lane == LANE_RIVER || prev_lane == LANE_CHASM) 
                        gen_lane_type = LANE_GRASS;
                    else 
                        gen_lane_type = LANE_CHASM;
                end
            endcase
        end
    endfunction
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            if (world_scrolled) begin
                prev_world_y <= prev_world_y + 11'd64;
                ...
                lane_types[0] <= gen_lane_type(lfsr[2:0], consecutive_danger, lane_types[0]);

                if (gen_lane_type(lfsr[2:0], consecutive_danger, lane_types[0]) == LANE_GRASS ||
                    gen_lane_type(lfsr[2:0], consecutive_danger, lane_types[0]) == LANE_START)
                    consecutive_danger <= 2'd0;
                else
                    consecutive_danger <= consecutive_danger + 2'd1;

                if (gen_lane_type(lfsr[2:0], consecutive_danger, lane_types[0]) == LANE_RIVER) begin
