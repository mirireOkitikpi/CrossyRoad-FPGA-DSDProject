`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// Module:      lane_manager.v (v3 - overwrite bug fixed)
// Description:
//   Procedurally generates and manages the Crossy Road lane map.
//   Fixes a register-overwrite bug where world scroll and horizontal
//   obstacle movement both wrote to lane_obs_x/lane_obs2_x in the
//   same clock cycle, causing the horizontal movement to silently
//   clobber the scroll shift. The two phases are now mutually
//   exclusive: on a scroll tick, ONLY the shift executes; on all
//   other ticks, ONLY horizontal movement executes.
//////////////////////////////////////////////////////////////////////////////////
...
module lane_manager (
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            // prev_world_y always updates - not a conflict
            prev_world_y <= logical_world_y;

            // mutually exclusive: scroll OR movement, never both
            if (world_scrolled) begin
                // Phase A: vertical lane shift 
                // This ENTIRELY replaces horizontal movement for this tick.
                // Obstacles "pause" for exactly one frame while the
                // world shifts - imperceptible at 60Hz.
                ...
                // Phase B: horizontal obstacle movement 
                // Only runs on ticks when no scroll is happening
                for (i = 0; i < NUM_LANES; i = i + 1) begin
            ...
            // If neither scroll nor game_active: all obstacle regs hold
            // their values naturally (no assignment = no change for FF)
        end
    end
